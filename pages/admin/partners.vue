  ...
  <main  class="center pb-2 adminHome" >
    ...
    <AdminModalWrap @closeDlg="closeModal" mWidth="1000px" origWidth="100%" :showDlg="showDlg">
      <div v-if="mode === 'edit'" class="flexCentered">
        <img :src="partnerToUpdate.image" alt="">
      </div>
      ...
      <div>
        ...
        <input type="checkbox" v-model="partnerToUpdate.status" id="status">
      </div>
...
useMeta({
  title: 'Панель управления - Партнеры'
})


const {data, error} = await useAsyncData('adminPartners', () => $fetch('/api/admin/partners/index'));
...
const partnerToUpdate = ref({status: false});
...
function closeModal() {
  showDlg.value = false;
  mode.value = null;
  partnerToUpdate.value = {status: false}
  selectedFile.value = null;
}

function updateItem(partner) {
  mode.value = 'edit';
  partnerToUpdate.value = {...partner}
  showDlg.value = true;
}
...
async function storeItem() {
  ...
  formData.append('data', JSON.stringify(partnerToUpdate.value))
  ...
  try {
    ...
    if (mode.value === 'edit') {
      const {result} = await $fetch('/api/admin/partners/edit', {
        method: 'POST',
        body: formData,
      })
      const ind = data.value.partners.findIndex(item => item.id === result.id);
      data.value.partners[ind] = result;
    }

    if (mode.value === 'add') {
      const {result} = await $fetch('/api/admin/partners/add', {
        method: 'POST',
        body: formData,
      })
      data.value.partners.unshift(result);
    }
  ...
  if (confirm('Are you sure?')) {
    try {
     ...
     // formData.append('url', url);
      ...
      const {id} = await $fetch('/api/admin/partners/remove', {
        method: 'POST',
        body: formData,
      })

      data.value.partners.splice(data.value.partners.findIndex(item => item.id === id), 1);
